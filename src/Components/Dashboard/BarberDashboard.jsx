...
export default function BarberDashboard() {
    ...
    function formatPrice(input) {
        const strInput = String(input); // гарантируем, что это строка
        const cleanedInput = strInput.includes('.') ? strInput.replace(/\.0+$/, '') : strInput;
            ...
            <div className="bg-white p-4 rounded-lg shadow">
                ...
                <div className="space-y-3">
                    {bronData?.map((i, index) => {
                        const totalPrice = i?.data?.services?.reduce((acc, item) => acc + Number(item?.price || 0), 0);
                        ...
                        return (
                            ...
                            >
                                ...
                                {i?.data?.services?.map((item, serviceIndex) => (
                                    <div key={serviceIndex} className="border-[1px] p-[10px] mt-[10px] rounded-[5px] ">
                                        <p className="text-sm text-gray-500">{serviceIndex + 1}. {' '}{item?.name_uz}</p>
                                        <p className="text-sm text-gray-500 mt-[10px]">{formatPrice(item?.price)} so'm</p>
                                    </div>
                                ))}
