...
export default function Sidebar() {
    ...
    return (
        <div className="h-[97%] w-[300px] shadow-2xl bg-white fixed mt-[10px] ml-[10px] p-[10px] rounded-[10px]">
            <div>
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcStsDCDW7bWAkuzwm1d2OF0rWYIFFkYAhBnhw&s" alt="Logo" className="mx-auto" />
            </div>
            <div className="mt-[30px]">
                {adminMenuItems.map((item) => (
                    <NavLink key={item.id} to={item.path}>
                        <button className={`group w-full ${item.id !== 1 ? 'mt-[10px]' : 'mb-[10px]'} shadow-md duration-500 hover:shadow-xl scale-95 hover:scale-100 flex items-center gap-[10px] border-[2px] rounded-[10px] border-[#E5EFF9] p-[5px] hover:border-[#E6F0F9] hover:bg-[#F0F8FF]`}>
                            {item.icon}
