...
export default function BarbersOne() {
    ...
    return(
        ...
        <div 
          ...
          onClick={() => setSelectedTab(selectedTab === 'specialist' ? null : 'specialist')}
        >
          ...
          <ChevronRight 
            className={`text-gray-400 transition-transform ${selectedTab === 'specialist' ? 'rotate-90' : ''}`} 
            size={20} 
          />
        </div>
        ...
        {selectedTab === 'specialist' && (
          <div className="mt-4 space-y-4 transition-all">
            ...
            <div 
              className={`p-4 rounded-lg shadow-sm cursor-pointer transition-all bg-white border border-gray-200 hover:border-gray-300 ${
                !selectedSpecialist ? 'border-black' : ''
              }`}
              ...
              <div className="flex justify-between items-center">
                ...
                <div
                  className={`h-6 w-6 rounded-full flex items-center justify-center ${
                    !selectedSpecialist ? 'bg-black' : 'border border-gray-300'
                  }`}
                >
                  {!selectedSpecialist && (
              ...
              >
                <div className="flex justify-between items-start  mb-3">
                  ...
                  <div className="flex items-center space-x-2 relative">
                    ...
                    <div
                      className={`h-6 w-6 rounded-full flex items-center justify-center ${
                        selectedSpecialist?.id === barber.id ? 'bg-black' : 'border border-gray-300'
                      }`}
                    >
                      {selectedSpecialist?.id === barber.id && (
                        <div className="h-2 w-2 rounded-full bg-white"></div>
                    ...
                    {barber.availableTimes.map((time, index) => (
                      <button 
                        ...
                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                          selectedTime?.time === time && selectedTime?.barberId === barber.id
                            ? 'bg-black text-white' 
